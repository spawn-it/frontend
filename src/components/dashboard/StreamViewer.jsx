...
const StreamViewer = ({ clientId, serviceId, colors }) => {
      ...
      try {
        ...
        eventSourceRef.current.addEventListener('end', event => {
          try {
            ...
            handleNewLog(
              `Command completed with status: ${statusData.applied ? 'SUCCESS' : 'FAILED'}`,
              'status'
            );
          } catch {
  ...
  const handleTogglePause = () => {
    setIsPaused(prev => {
      if (prev) {
        setLogs(prevLogs =>
          [...prevLogs, ...pausedLogsRef.current].slice(-1000)
        );
        pausedLogsRef.current = [];
      }
      return !prev;
  ...
  const handleDownloadLogs = () => {
    ...
    const logText = allLogs
      .map(log => `[${log.timestamp}] ${decodeEscapedChars(log.message)}`)
      .join('\n');
