...
const AdvancedConfiguration = ({ deployConfig, updateConfig }) => {
  ...
  const portList = useMemo(() => {
    ...
    return Object.entries(deployConfig.ports || {}).map(
      ([exposed, internal], index) => ({
        id: `port-${index}`,
        exposed,
        internal,
      })
  ...
  const envVarList = useMemo(() => {
    ...
    return Object.entries(deployConfig.env_vars || {}).map(
      ([key, value], index) => ({
        id: `env-${index}`,
        key,
        value,
  ...
  const updatePort = (targetId, field, value) => {
    const newList = portList.map(port =>
      port.id === targetId ? { ...port, [field]: value } : port
    );
    updatePorts(newList);
  };
  ...
  const updateEnvVar = (targetId, field, value) => {
    const newList = envVarList.map(envVar =>
      envVar.id === targetId ? { ...envVar, [field]: value } : envVar
    );
    updateEnvVars(newList);
  };
  ...
  return (
    <Box sx={{ mt: 2, mb: 4 }}>
      ...
      <Alert
        severity="info"
        sx={{ mb: 4, bgcolor: colors.paper, color: colors.text }}
      >
        La configuration par défaut est prête à l'emploi. Modifiez les ports ou
        les variables{' '}
        <strong>uniquement si vous savez ce que vous faites</strong>.
      </Alert>
